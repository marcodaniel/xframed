...
\Pack{xframed} uses the commmand \Cmd{newxframedstyle} and
provides a warning.
...
\begin{ltxexample}[caption=Example of helper functions,label=helper,result=true,]
 \begin{xframed}[margin=3cm]
    \centering
    \xframedsetup{inner-left-margin=1cm}
    \showxframendskip{skip-above}%
    \rule{\usexframendlength{inner-left-margin}}{2pt}%
...
\ExplOpt[true]{show-all-lines}%metakeys
...
\ExplOpt[true]{top-line}%bool-option
The option \Opt{show-all-lines} controls the behaviour
of all lines. But yu can control every line separat. The option 
\Opt{top-line} influence the line above the frame. It's also 
a boolean key and accepts either \texttt{true} or \texttt{false}.

\ExplOpt[true]{left-line}%bool-option
It's similar to \Opt{top-line} but it controls the left line.

\ExplOpt[true]{bottom-line}%bool-option
It's similar to \Opt{top-line} but it controls the bottom line.
\ExplOpt[true]{right-line}%bool-option
It's similar to \Opt{top-line} but it controls the right line.

\ExplOpt{round-corner}%metakeys
This is a meta key. That means the value of \Opt{round-corner} is passed
to the options \Opt{outer-arc} and \Opt{inner-arc}.

\ExplOpt{arc}
It's a short synonym for \Opt{round-cornerns}.

\ExplOpt[10pt]{outer-arc}
The frame around the provided material of \Env{xframed} 
has a defined width. The outer part of the line gets
rounded corners by the dimension of the value by \Opt{outer-arc}.
...
\ExplTOpt[rounded~corners = \Opt{inner-arc}]{inner~arc/.style}
This style controls inner arc of the frame.
\ExplTOpt[rounded~corners = \Opt{outer-arc}]{outer~arc/.style}
